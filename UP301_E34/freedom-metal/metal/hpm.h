...
/*! @brief Enums for HPM return codes */
typedef enum enum_hpm_retcode{
    E_HPM_SUCCESS,     // Success.
	E_HPM_INVINIT,	 	// Initial fail or resource is unavailable.
	E_HPM_INVPARA,	 	// Invalid function parameter/argument.
    E_HPM_ERROR,     	// Unspecific error.
} enum_hpm_retcode_Type;

/*! @brief Initialize hardware performance monitor counters.
 * @param cpu The CPU device handle.
 * @return enum_hpm_retcode_Type error call.*/
enum_hpm_retcode_Type metal_hpm_init(struct metal_cpu *cpu);

/*! @brief Disables hardware performance monitor counters.
 *         Note - Disabled HPM counters may reduce power consumption.
 * @param cpu The CPU device handle.
 * @return enum_hpm_retcode_Type error call.*/
enum_hpm_retcode_Type metal_hpm_disable(struct metal_cpu *cpu);
 ...
 * @return enum_hpm_retcode_Type error call.*/
enum_hpm_retcode_Type metal_hpm_set_event(struct metal_cpu *cpu, metal_hpm_counter counter,
									             uint32_t bitmask);
...
uint32_t metal_hpm_get_event(struct metal_cpu *cpu,
 ...
 * @param counter Hardware counter.
 * @return enum_hpm_retcode_Type error call.*/
enum_hpm_retcode_Type metal_hpm_clr_event(struct metal_cpu *cpu, metal_hpm_counter counter,
									uint32_t bitmask);
 ...
 * @param counter Hardware counter.
 * @return enum_hpm_retcode_Type error call.*/
enum_hpm_retcode_Type metal_hpm_enable_access(struct metal_cpu *cpu, metal_hpm_counter counter);

/*! @brief Disable counter access to next lower privilege mode.
 * @param cpu The CPU device handle.
 * @param counter Hardware counter.
 * @return enum_hpm_retcode_Type error call.*/
enum_hpm_retcode_Type metal_hpm_disable_access(struct metal_cpu *cpu, metal_hpm_counter counter);
...
uint64_t metal_hpm_read_counter(struct metal_cpu *cpu,
                                       metal_hpm_counter counter);
 ...
 * @param counter Hardware counter.
 * @return enum_hpm_retcode_Type error call.*/
enum_hpm_retcode_Type metal_hpm_clear_counter(struct metal_cpu *cpu, metal_hpm_counter counter);
