...
typedef enum _mmc_standby_transfer
{
    kMMC_Standby = 1U, /*!< MMC card sleep */
    kMMC_Transfer = 0U, /*!< MMC card awake */
} mmc_standby_transfer_t;

/*! @brief mmccard speed mode HS200/DDR50 */
typedef enum _mmc_speed_mode
{
	HS200_mode = 1U, /*!< High Speed 200 */
	DDR50_mode = 0U, /*!< Double Data Rate 50 */
} mmc_speed_mode_t;

/*! @brief card io strength control */
typedef void (*mmc_io_strength_t)(sdmmchost_t *host, uint32_t busFreq);
...
typedef enum _usdhc_wp_mode
{
    kUSDHC_TemporaryWriteProtect = 0U, 
    kUSDHC_PoweronWriteProtect = 1U, 
}_usdhc_wp_mode_t;
...
typedef struct _mmc_card
{
    ...
    uint32_t writeProtectGroupBlocks;                            /*!< Write protect group size united as block size */
	...
	mmc_speed_mode_t speedmode;					/*!< card speed rate */
	bool isIsrMode;           /*!< Use this flag to determine whether need a interrupt or not (Bill added)*/
 ...
 */
int MMC_Init(mmc_card_t *card);

int MMC_Tuning(mmc_card_t *card);
/*!
 ...
 * @retval #kStatus_Success Operation succeeded.
 */
int MMC_CardInit(mmc_card_t *card);
 ...
 * @param card Card descriptor.
 */
int MMC_HostInit(mmc_card_t *card);
 ...
 * @retval #kStatus_Success Operation succeeded.
 */
int MMC_ReadBlocks(mmc_card_t *card, uint8_t *buffer, uint32_t startBlock, uint32_t blockCount);
 ...
 * @retval #kStatus_Success Operation succeeded.
 */
int MMC_WriteBlocks(mmc_card_t *card, const uint8_t *buffer, uint32_t startBlock, uint32_t blockCount);
 ...
 * @retval #kStatus_Success Operation succeeded.
 */
int MMC_EraseGroups(mmc_card_t *card, uint32_t startGroup, uint32_t endGroup, mmc_erase_t mode);
 ...
 * @retval #kStatus_Success Operation succeeded.
 */
int MMC_SelectPartition(mmc_card_t *card, mmc_access_partition_t partitionNumber);
 ...
 * @retval #kStatus_Success Operation succeeded.
 */
int MMC_SetBootConfig(mmc_card_t *card, const mmc_boot_config_t *config);
 ...
 * @retval #kStatus_Success Operation succeeded.
 */
int MMC_StartBoot(mmc_card_t *card,
 ...
 */
int MMC_SetBootConfigWP(mmc_card_t *card, uint8_t wp);
 ...
 * @param hostConfig Host boot configurations.
 */
int MMC_ReadBootData(mmc_card_t *card, uint8_t *buffer, sdmmchost_boot_config_t *hostConfig);
 ...
 */
int MMC_StopBoot(mmc_card_t *card, uint32_t bootMode);
 ...
 */
int MMC_SetBootPartitionWP(mmc_card_t *card, mmc_boot_partition_wp_t bootPartitionWP);
 ...
 */
int MMC_EnableCacheControl(mmc_card_t *card, bool enable);
 ...
 * @param card Card descriptor.
 */
int MMC_FlushCache(mmc_card_t *card);
 ...
 */
int MMC_SetSleepAwake(mmc_card_t *card, mmc_sleep_awake_t state);
 ...
 */
int MMC_PollingCardStatusBusy(mmc_card_t *card, bool checkStatus, uint32_t timeoutMs);
status_t ClearWriteProtectGroup(mmc_card_t *card, uint32_t groupAddr);
status_t CheckWriteProtectType(mmc_card_t *card, uint32_t groupAddr, uint8_t *buffer);
status_t SetWriteProtectGroup(mmc_card_t *card, uint32_t groupAddr);
uint32_t SetWriteProtect(mmc_card_t *card, _usdhc_wp_mode_t mode);
uint32_t MMC_Pon(mmc_card_t *card);
uint32_t MMC_Check_Status(mmc_card_t *card);
uint32_t MMC_SetStandbyTransfer(mmc_card_t *card, mmc_standby_transfer_t state);
uint32_t MMC_Discard(mmc_card_t *card, uint32_t startgroup, uint32_t endgroup);
uint32_t MMC_SecureErase(mmc_card_t *card, uint32_t startgroup, uint32_t endgroup);
uint32_t MMC_SecureTrim(mmc_card_t *card, uint32_t startgroup, uint32_t endgroup);
uint32_t MMC_Trim(mmc_card_t *card, uint32_t startgroup, uint32_t endgroup);
uint32_t MMC_Sanitize(mmc_card_t *card);
uint32_t MMC_Check_Status(mmc_card_t *card);
uint32_t MMC_Send_Status(mmc_card_t *card);
uint32_t MMC_Poff(mmc_card_t *card);
/* @} */
#if defined(__cplusplus)
}
#endif
