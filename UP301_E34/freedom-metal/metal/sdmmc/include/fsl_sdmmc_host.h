...
#define SDMMCHOST_SUPPORT_MAX_BLOCK_LENGTH     (512U)	// Mimi
...
#else	// Mimi: removed
#define SDMMCHOST_STANDARD_TUNING_START            (0x1U) /*!< standard tuning start point */
#define SDMMCHOST_TUINIG_STEP                      (2U)  /*!< standard tuning stBep */
#endif
...
typedef struct _sdmmchost_
{
    ...
    bool dmaDisable;               /* with/without DMA */
} sdmmchost_t;
...
static inline void SDMMCHOST_EnableCardInt(sdmmchost_t *host, bool enable)
{
    ...
    if (enable)
    {
        USDHC_EnableInterruptStatus(host->hostController.base, ((uint32_t)kUSDHC_CommandFlag |
                                    (uint32_t)kUSDHC_DataDMAFlag | (uint32_t)kUSDHC_TuningErrorFlag));
        USDHC_EnableInterruptSignal(host->hostController.base, ((uint32_t)kUSDHC_CommandFlag |
                                    (uint32_t)kUSDHC_DataDMAFlag | (uint32_t)kUSDHC_TuningErrorFlag));
    }
    else
    {
        USDHC_DisableInterruptStatus(host->hostController.base, ((uint32_t)kUSDHC_CommandFlag |
                                    (uint32_t)kUSDHC_DataDMAFlag | (uint32_t)kUSDHC_TuningErrorFlag));
        USDHC_DisableInterruptSignal(host->hostController.base, ((uint32_t)kUSDHC_CommandFlag |
                                    (uint32_t)kUSDHC_DataDMAFlag | (uint32_t)kUSDHC_TuningErrorFlag));
    }
}
