...
//#define SWAP_WORD_BYTE_SEQUENCE(x) (__REV(x))	// Mimi
#define SWAP_WORD_BYTE_SEQUENCE(x) (((0xff000000 & x) >> 24) | 	\
									((0x00ff0000 & x) >> 8) |  	\
									((0x0000ff00 & x) << 8) |	\
									((0x000000ff & x) << 24))
/*! @brief Reverse byte sequence for each half word in uint32_t */
//#define SWAP_HALF_WROD_BYTE_SEQUENCE(x) (__REV16(x))	// Mimi
#define SWAP_HALF_WROD_BYTE_SEQUENCE(x) (((0xff00 & x) >> 8) | 	\
										 ((0x00ff & x) << 8))
...
// #define FSL_SDMMC_CARD_INTERNAL_BUFFER_ALIGN_ADDR(buffer)                     \
//     (uint32_t)(((uint32_t)(buffer) + (uint32_t)SDMMC_DATA_BUFFER_ALIGN_CACHE - 1U)\
//                & ~((uint32_t)SDMMC_DATA_BUFFER_ALIGN_CACHE - 1U))
...
typedef void (*sd_io_strength_t)(sdmmchost_t *host, uint32_t busFreq);
