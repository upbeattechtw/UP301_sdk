 ...
 * This software contains code derived from software originally developed by Nordic Semiconductor ASA.
    ...
    while (it->tlv_off < it->tlv_end) {
        ...
        if (it->type == IMAGE_TLV_ANY || tlv.it_type == it->type) {
            ...
            *len = tlv.it_len;
            ...
            /* Note: Potential mcuboot bug — dereferencing `*type` when `type` is NULL.
             * Solution: replace `*type` with `it->type`. */
